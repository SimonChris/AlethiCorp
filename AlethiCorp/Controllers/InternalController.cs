...
namespace AlethiCorp.Controllers
{
  public class InternalController : InternalLayoutController
  {
    ...
    public ActionResult Comply()
    {
      ...
      var progression = state != null ? state.GameProgression : GameProgression.Ongoing;
    ...
    public ActionResult Bear()
    {
      var state = db.GameStates.Where(s => s.UserName == User.Identity.Name).SingleOrDefault();
      var progression = state != null ? state.GameProgression : GameProgression.Ongoing;
      if (progression != GameProgression.Bear && progression != GameProgression.BearBearBear)
      {
        return HttpNotFound();
      }
      if (progression != GameProgression.BearBearBear)
      {
        state.GameProgression = GameProgression.BearBearBear;
        db.Entry(state).State = EntityState.Modified;
        db.SaveChanges();
      }
